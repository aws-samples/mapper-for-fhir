...
module.exports.external = [
    ...
    {
	name: 'collapseOBXtxt',
	description:"collapses all TX OBX segments into one",
	func: function collapseOBXtxt(msg) {
		var totalText = "";
			for (var i = 0; i < msg.meta.length; i++){
				if (msg.meta[i] == 'OBX'){
					if (msg.data[i].toString().split(",")[1] == 'TX') {
						totalText = totalText + msg.data[i].toString().split(",")[5] + " ";
					}	
				}
			}
        ...
        func: function getRelatedSegmentList(msg, parentSegment, parentIndex, childSegment) {
            try {
                ...
                if (childSegment == 'OBX') {
                    if (childIndex > -1) {
                        do {
                            if (msg.data[childIndex] && (msg.meta[childIndex] == childSegment)) {
                                segOut.push(msg.data[childIndex]);
                            }
                            childIndex++;
                            //console.log("First IF: " + parentSegment + " " +  parentIndex + " " +childSegment + " " + childIndex);
                        } while (childIndex < msg.meta.length);
                    }
                } else {
                    if (childIndex > -1) {
                        do {
                            ...
                            childIndex++;
                            //console.log("Second IF: " + parentSegment + " " +  parentIndex + " " +childSegment + " " + childIndex)
    ...
    },
   {
    ...
    },
    {
        name: 'generateUUID',
        description: 'Generates a guid based on a URL: generateUUID url',
	func: function (urlNamespace, leaveDashes = 0) {
		if (leaveDashes == 1) return uuidv3(''.concat(urlNamespace), uuidv3.URL); else
        	return uuidv3(''.concat(urlNamespace), uuidv3.URL).replace(new RegExp('-', 'g'), '');
        }
    },
    {
        name: 'generateUUIDV2',
